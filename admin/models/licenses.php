 ...
 * @since  1.6
 */
class PixextModelLicenses extends JModelList
{
/**
	* Constructor.
	*
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('a.pixext_license_id', 'asc');
	}
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__pixext_licenses` AS a');

		// Join over the users for the checked out user
		$query->select("uc.name AS uEditor");
		$query->join("LEFT", "#__users AS uc ON uc.id=a.checked_out");
		// Join over the foreign key 'pixext_extension_id'
		$query->select('`#__pixext_extensions_2663456`.`name` AS extensions_fk_value_2663456');
		$query->join('LEFT', '#__pixext_extensions AS #__pixext_extensions_2663456 ON #__pixext_extensions_2663456.`pixext_extension_id` = a.`pixext_extension_id`');
		...
		if (!empty($search))
		{
			if (stripos($search, 'pixext_license_id:') === 0)
			{
				$query->where('a.pixext_license_id = ' . (int) substr($search, 3));
			}
			else
			{
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->pixext_extension_id))
			{
				...
				foreach ($values as $value){
					...
					$query = $db->getQuery(true);
					$query
							->select('`#__pixext_extensions_2663456`.`name`')
							->from($db->quoteName('#__pixext_extensions', '#__pixext_extensions_2663456'))
